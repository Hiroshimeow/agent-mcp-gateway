...
DeviceBroker uses `/device` WebSocket upgrades with durable Ed25519 device identity. `MCP_DEVICE_ENROLLMENT_TOKEN` authorizes first-time enrollment only; the broker stores only the public key in `.runtime/devices.sqlite`, and subsequent reconnects use a broker challenge signed by the device private key. Remote execution is deny-by-default: `MCP_DEVICE_ACCESS_POLICY` must explicitly match caller, device, tool, and allowed roots; `list_devices` is filtered by the same ACL without changing the stable tool schema. Remote calls enforce bounded request/output sizes and per-rule request rates, while `.runtime/device-audit.jsonl` stores metadata only (hashed caller id, device id, tool, timing, sizes, outcome/error code). Pre-enroll an operator-approved Ed25519 public key with `npm run device:enroll -- <device_id> <public_key.pem>`, rotate an active device key with compare-and-swap protection using `npm run device:rotate -- <device_id> <expected_current_public_key.pem> <new_public_key.pem>`, or revoke an enrolled identity with `npm run device:revoke -- <device_id>`. Rotation/revocation invalidates stale authorization before new calls or results are accepted, and displaced in-flight requests fail without replay. Rollback is another guarded rotation to a previously trusted, non-compromised public key; revocation remains authoritative, and private keys are never retained in the registry.
