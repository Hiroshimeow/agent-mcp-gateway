...
DeviceBroker uses `/device` WebSocket upgrades with durable Ed25519 device identity. `MCP_DEVICE_ENROLLMENT_TOKEN` authorizes first-time enrollment only; the broker stores only the public key in `.runtime/devices.sqlite`, and subsequent reconnects use a broker challenge signed by the device private key. `list_devices` remains a stable read-only MCP tool, and device inventory changes do not change the tool schema. Revoke an enrolled identity with `npm run device:revoke -- <device_id>`.
