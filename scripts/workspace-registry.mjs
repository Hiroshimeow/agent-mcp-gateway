...
export function createWorkspaceRegistry(options = {}) {
  ...
  function buildState(content, runtimeContent = '') {
    ...
    const staticRoots = rootsFromRawConfig(rawConfig, { repoRoot, env, platform }).map(item => item.path);
    const runtimeRoots = rootsFromRawConfig(runtimeRawConfig, { repoRoot, env, platform }).map(item => item.path)
