...
function rowToDevice(row) {
  ...
  return {
    ...
    minProtocol: Number(row.min_protocol || 1)
  };
}
...
export function createDeviceStore({ dbPath, now = () => new Date().toISOString() }) {
  ...
  if (!columns.some(column => column.name === 'min_protocol')) {
    db.exec('ALTER TABLE devices ADD COLUMN min_protocol INTEGER NOT NULL DEFAULT 1');
  }
  ...
  for (const column of ['hostname', 'platform', 'arch', 'path_style', 'agent_version', 'package_version']) {
    if (!columns.some(entry => entry.name === column)) db.exec(`ALTER TABLE devices ADD COLUMN ${column} TEXT`);
  }
  db.exec('PRAGMA busy_timeout = 5000');

  const getStatement = db.prepare(`
    SELECT device_id, device_name, owner_account_id, account_label, hostname, platform, arch, path_style, agent_version, package_version, public_key_pem, enrolled_at, revoked_at, authorization_generation, min_protocol
    FROM devices
    WHERE device_id = ?
  `);
  ...
  const insertStatement = db.prepare(`
    INSERT INTO devices (device_id, device_name, owner_account_id, account_label, hostname, platform, arch, path_style, agent_version, package_version, public_key_pem, enrolled_at, revoked_at, authorization_generation, min_protocol)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 1, ?)
  `);
  ...
  const metadataStatement = db.prepare(`
    UPDATE devices
    SET device_name = ?, account_label = ?, hostname = ?, platform = ?, arch = ?, path_style = ?, agent_version = ?, package_version = ?
    WHERE device_id = ? AND revoked_at IS NULL
  `);
  const ownerStatement = db.prepare(`
    ...
    WHERE device_id = ? AND revoked_at IS NULL
  `);
  const raiseProtocolFloorStatement = db.prepare(`
    UPDATE devices
    SET min_protocol = ?
    WHERE device_id = ? AND min_protocol < ?
  `);
  ...
  function enroll({ deviceId, publicKeyPem, deviceName = null, ownerAccountId = null, accountLabel = null, hostname = null, platform = null, arch = null, pathStyle = null, agentVersion = null, packageVersion = null, minProtocol = 1 }) {
    ...
    const normalizedAgentVersion = String(agentVersion || '').trim().slice(0, 64) || null;
    const normalizedPackageVersion = String(packageVersion || '').trim().slice(0, 32) || null;
    const normalizedMinProtocol = Math.max(1, Number(minProtocol) || 1);
    if (getStatement.get(normalizedId)) throw new Error(`Device ${normalizedId} is already enrolled.`);
    const enrolledAt = now();
    insertStatement.run(normalizedId, normalizedName, normalizedOwner, normalizedAccount, normalizedHostname, normalizedPlatform, normalizedArch, normalizedPathStyle, normalizedAgentVersion, normalizedPackageVersion, normalizedKey, enrolledAt, normalizedMinProtocol);
    return {
      ...
      minProtocol: normalizedMinProtocol
    };
  }
  ...
  function updateMetadata({ deviceId, deviceName, accountLabel, hostname, platform, arch, pathStyle, agentVersion, packageVersion }) {
    const normalizedId = normalizeDeviceId(deviceId);
    ...
    const nextAgentVersion = agentVersion === undefined ? current.agentVersion : (String(agentVersion || '').trim().slice(0, 64) || null);
    const nextPackageVersion = packageVersion === undefined ? current.packageVersion : (String(packageVersion || '').trim().slice(0, 32) || null);
    const result = metadataStatement.run(nextName, nextAccount, nextHostname, nextPlatform, nextArch, nextPathStyle, nextAgentVersion, nextPackageVersion, normalizedId);
  ...
  function raiseProtocolFloor(deviceId, minProtocol) {
    const normalizedId = normalizeDeviceId(deviceId);
    const floor = Math.max(1, Number(minProtocol) || 1);
    const current = get(normalizedId);
    if (!current) throw new Error(`Unknown device ${normalizedId}.`);
    raiseProtocolFloorStatement.run(floor, normalizedId, floor);
    return get(normalizedId);
  }
