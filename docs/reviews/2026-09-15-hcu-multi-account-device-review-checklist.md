...
Evidence: latest gateway `9e8edd7193bfc5318524d1dafed00cdf4f5baa10` verified 282/282 plus three MCP smokes; the reduced test count reflects deletion of the dead runtime trusted-root auto-grant subsystem and its dedicated filesystem fixture tests. Latest device `1cfc497b33155f99e0f68e6059780fc3f9c68289` verified 52/52 plus build/package PASS. Real post-cleanup npm-pack smoke: 727,748-byte / 244-file tarball, `hcu-device -> dist/hcu-device.js`, isolated `status --json` exit 0, no live identity access, and no unused direct `glob`. Both repos scanned 0 tracked personal-path and 0 secret-like files. Desktop Commander MIT/legal attribution and upstream remote remain. Final debt/ponytail review removed the legacy shared-password launcher alias, unused `mcp-proxy`, test-only filesystem MCP package, consumerless runtime root overlay/locking/persistence code (42 insertions / 1,450 deletions), and device direct `glob`, while preserving tested static TOML hot reload/external MCP reconcile. ExcelJS transitive deprecations remain recorded upstream debt rather than speculative replacement. No speculative HA/watcher infrastructure was added. Final stable model surface: 16 tools / 15,297 bytes / ~3,825 estimated tokens versus 16 / 16,225 / ~4,057 baseline.
