...
- [x] Create local admin via CLI on the v2 host.
...
- [x] Cut over OAuth/device pairing only after a normal account has been created interactively.
- [x] Re-pair/re-enroll ThinkBook under a normal account and confirm a second Windows device can pair under another normal account.
- [ ] Confirm access-token expiry triggers refresh, not interactive login, while valid refresh token remains; automated refresh-rotation coverage is green, but live ChatGPT OAuth acceptance remains the final runtime proof.
- [x] Confirm hport/cloudflared process health separately from device WSS reconnect health.

**Current checkpoint:** port 8102 is running the account-aware build directly and temporary 8103 is removed. Normal product accounts now exist and live device pairing has succeeded: ThinkBook was paired to `thuhang91199+phase8@gmail.com`, and a second Windows device (`lppduongnh66-1-68b4806c`, friendly name `fjp`) was paired to `ssnouru@gmail.com`, installed as a background service, and reported online with the production 16-tool / 15,297-byte schema. Public signup/login/dashboard remain live; a post-signup redirect defect that sent successful registration to the undefined `/` route was reproduced, fixed to `/dashboard`, covered by regression tests, and deployed to public v2. OAuth metadata advertises `mcp:tools` + `offline_access`; automated coverage verifies account-bound access/refresh tokens and refresh-token rotation. During cutover a reconnect storm was previously traced to stale `remote --help` processes; device commit `4bb56c4dc6b820b3227f101f99b2b85546de7bda` prevents help from starting a runtime. Old v1 remains intentionally untouched.

**Gate:** operational pairing/cutover is complete. Final live acceptance is now the ChatGPT custom-app OAuth/tool-scan path: authenticate a normal HCU account, confirm account-scoped `list_devices` plus one safe device call, confirm non-owned devices remain hidden/denied, and observe refresh continuity when ChatGPT stores the issued refresh token. Automated denial and refresh-rotation coverage is already green; stale legacy metadata cleanup remains housekeeping after this live client acceptance.
...
**Gate:** release-readiness cleanup is verified. Overall completion now depends only on final live ChatGPT custom-app OAuth/tool-scan acceptance and post-acceptance stale legacy metadata cleanup; normal-account signup and multi-device pairing are complete.
