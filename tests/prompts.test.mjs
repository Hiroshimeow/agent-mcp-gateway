...
test('getRepoPrompt returns MCP-shaped messages', () => {
  const prompt = getRepoPrompt('quality_check', { device_id: 'device-a', project_id: 'fixture' }, { safetyProfile: { name: 'yolo' } });
...
test('security and cross-platform prompts mention required caveats', () => {
  assert.match(getRepoPrompt('quality_check', { device_id: 'device-a', project_id: 'fixture' }).messages[0].content.text, /architectural health/);
  assert.match(getRepoPrompt('cross_platform_review', { device_id: 'device-a', project_id: 'fixture' }).messages[0].content.text, /POSIX non-login -c/);
  assert.match(getRepoPrompt('release_readiness', { device_id: 'device-a', project_id: 'fixture' }).messages[0].content.text, /untracked imported files/);
});
