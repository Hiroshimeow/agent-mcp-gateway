...
import { AccountAuthProvider, FileBackedAuthState } from '../scripts/auth-session.mjs';
...
function fixture() {
  ...
  const stateStore = new FileBackedAuthState(path.join(dir, 'auth-state.json'));
...
test('OAuth access and refresh tokens preserve account_id and revoke with the account', async () => {
  ...
  try {
    ...
    const restarted = new AccountAuthProvider({
      stateStore: new FileBackedAuthState(path.join(f.dir, 'auth-state.json')),
